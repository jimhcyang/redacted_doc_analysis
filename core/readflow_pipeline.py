...
PERCENTILE_Q_X_DEFAULT = 0.5
PERCENTILE_Q_Y_DEFAULT = 0.0
VERTICAL_TEXTBOX_BUFFER_RATIO = 0.01
ROW_EMPTY_RATIO_FOR_GAP = 0.841
ROW_INK_THRESHOLD_DEFAULT = 1.0 - ROW_EMPTY_RATIO_FOR_GAP  # 0.159
TINY_LINE_MERGE_RATIO_OF_MEDIAN = 1.0 / 3.0
PARAGRAPHBLANK_MIN_HEIGHT_MULTIPLIER = 1.5
...
def percentile_region(
    mask: np.ndarray,
    qx: float = PERCENTILE_Q_X_DEFAULT,
    qy: float = PERCENTILE_Q_Y_DEFAULT,
    ...
    qx = float(max(0.0, min(49.9, qx)))
    qy = float(max(0.0, min(49.9, qy)))
    left = int(np.floor(np.percentile(xs, qx)))
    right = int(np.ceil(np.percentile(xs, 100.0 - qx)))
    top = int(np.floor(np.percentile(ys, qy)))
    bottom = int(np.ceil(np.percentile(ys, 100.0 - qy)))
    ...
    # Add 1% vertical buffer on both sides of detected text box height.
    box_h = max(1, bottom - top + 1)
    pad_y = max(1, int(round(VERTICAL_TEXTBOX_BUFFER_RATIO * box_h)))
    top = max(0, top - pad_y)
...
def _row_ink_ratio_excluding_wide_white_runs(
    roi_smooth: np.ndarray,
    indentation_px: int,
    min_edge_black_points: int = 2,
) -> np.ndarray:
    ink = roi_smooth > 0
    if ink.size == 0:
    ...
    h, w = ink.shape
    indent_w = max(1, min(int(indentation_px), w))
    out = np.zeros(h, dtype=np.float32)

    for r in range(h):
        row = ink[r]
        black_count = int(np.count_nonzero(row))
        denom = int(w)

        # Apply white-chunk subtraction only when an edge probe has multiple ink points.
        left_black = int(np.count_nonzero(row[:indent_w]))
        right_black = int(np.count_nonzero(row[w - indent_w :]))
        apply_exclusion = (left_black >= min_edge_black_points) or (right_black >= min_edge_black_points)

        if apply_exclusion:
            white_run_exclusion_threshold = float(w) / 3.0
            c = 0
            while c < w:
                if row[c]:
                    c += 1
                    continue
                s = c
                while c + 1 < w and (not row[c + 1]):
                    c += 1
                run_len = c - s + 1
                if float(run_len) > white_run_exclusion_threshold:
                    denom -= run_len
                c += 1

        denom = max(1, denom)
        out[r] = float(black_count) / float(denom)

    return out


def _coalesce_adjacent_bands(
    ...
    for b in bands:
        ...
        y2 = int(b["y2"])
        if not out:
            out.append({"kind": kind, "y1": y1, "y2": y2, "box": (left, y1, right, y2)})
            continue
        prev = out[-1]
        if str(prev["kind"]) == kind and y1 <= int(prev["y2"]) + 1:
            prev["y2"] = max(int(prev["y2"]), y2)
            prev["box"] = (left, int(prev["y1"]), right, int(prev["y2"]))
        else:
            out.append({"kind": kind, "y1": y1, "y2": y2, "box": (left, y1, right, y2)})
    return out


def _merge_tiny_lines_with_previous(
    bands: list[dict[str, object]],
    left: int,
    right: int,
    tiny_ratio: float = TINY_LINE_MERGE_RATIO_OF_MEDIAN,
) -> list[dict[str, object]]:
    work = [
        {
            "kind": str(b["kind"]),
            "y1": int(b["y1"]),
            "y2": int(b["y2"]),
            "box": (int(left), int(b["y1"]), int(right), int(b["y2"])),
        }
        for b in bands
    ]
    if not work:
    ...
    while True:
        line_heights = [max(1, int(b["y2"]) - int(b["y1"]) + 1) for b in work if str(b["kind"]) == "line"]
        if not line_heights:
            break

        median_line_h = float(np.median(np.array(line_heights, dtype=np.float32)))
        tiny_line_h = max(1.0, float(tiny_ratio) * median_line_h)

        merge_idx: int | None = None
        for i in range(2, len(work)):
            b_prev_line = work[i - 2]
            b_gap = work[i - 1]
            b_line = work[i]
            if str(b_prev_line["kind"]) != "line":
                continue
            if str(b_gap["kind"]) != "gap":
                continue
            if str(b_line["kind"]) != "line":
                continue
            h_line = max(1, int(b_line["y2"]) - int(b_line["y1"]) + 1)
            if float(h_line) < float(tiny_line_h):
                merge_idx = i
                break
        ...
        prev_line = work[merge_idx - 2]
        tiny_line = work[merge_idx]
        prev_line["y2"] = int(tiny_line["y2"])
        prev_line["box"] = (left, int(prev_line["y1"]), right, int(prev_line["y2"]))

        # Remove the gap between lines and the tiny line itself.
        del work[merge_idx - 1 : merge_idx + 1]

    return _coalesce_adjacent_bands(work, left=left, right=right)


def detect_line_and_gap_bands(
    text_pixels_mask: np.ndarray,
    text_region: tuple[int, int, int, int],
    row_ink_threshold: float = ROW_INK_THRESHOLD_DEFAULT,
    indentation_px: int = 8,
    ...
    # A row is treated as a linebreak row if it is >=84.1% black (ink ratio <=15.9%).
    roi_smooth = cv2.dilate(roi, cv2.getStructuringElement(cv2.MORPH_RECT, (1, 3)), iterations=1)
    row_ratio = _row_ink_ratio_excluding_wide_white_runs(
        roi_smooth=roi_smooth,
        indentation_px=indentation_px,
    )
    ...
    # Merge tiny lines upward: line-gap-tiny-line becomes one larger line.
    bands = _merge_tiny_lines_with_previous(bands, left=left, right=right)
...
def build_output_line_model(
    ...
    linebreak_threshold = max(1, int(round(0.841 * avg_line_h)))
    ...
    for b_idx, band in enumerate(bands):
        ...
        if new_paragraph and is_blankline_gap:
            paragraphblank_height_px = int(max(1, round(PARAGRAPHBLANK_MIN_HEIGHT_MULTIPLIER * avg_line_h)))
        elif new_paragraph:
...
def process_image(
    ...
    percentile_q_x: float,
    percentile_q_y: float,
    first_line_indent_spaces: int,
    box_label_mode: str,
    box_line_chars: int,
) -> None:
    ...
    if np.count_nonzero(text_pixels_mask) > 0:
        text_region = percentile_region(text_pixels_mask, qx=percentile_q_x, qy=percentile_q_y)
    else:
        text_region = percentile_region(merged_region_mask, qx=percentile_q_x, qy=percentile_q_y)

    line_width_px = max(1, text_region[2] - text_region[0] + 1)
    row_ink_indentation_px = max(1, int(round((5.0 / max(1, box_line_chars)) * line_width_px)))

    line_band_boxes, gap_band_boxes, bands = detect_line_and_gap_bands(
        text_pixels_mask=text_pixels_mask,
        text_region=text_region,
        indentation_px=row_ink_indentation_px,
    )
        ...
        json.dumps(
            {
                ...
                "percentile_q_x": percentile_q_x,
                "percentile_q_y": percentile_q_y,
                ...
                "params": {
                    ...
                    "paragraphblank_min_height_multiplier": PARAGRAPHBLANK_MIN_HEIGHT_MULTIPLIER,
                    "row_ink_threshold": ROW_INK_THRESHOLD_DEFAULT,
                    "row_ink_indentation_px": int(row_ink_indentation_px),
                    "tiny_line_merge_ratio_of_median": TINY_LINE_MERGE_RATIO_OF_MEDIAN,
                    "first_line_indent_spaces": first_line_indent_spaces,
...
def main() -> None:
    ...
    parser.add_argument(
        "--percentile_q_x",
        type=float,
        default=PERCENTILE_Q_X_DEFAULT,
        help="Horizontal percentile trim for text region (left/right)",
    )
    parser.add_argument(
        "--percentile_q_y",
        type=float,
        default=PERCENTILE_Q_Y_DEFAULT,
        help="Vertical percentile trim for text region (top/bottom)",
    )
    parser.add_argument(
        "--percentile_q",
        type=float,
        default=None,
        help="Legacy uniform percentile applied to both axes when provided",
    )
    parser.add_argument(
    ...
    if args.percentile_q is not None:
        args.percentile_q_x = float(args.percentile_q)
        args.percentile_q_y = float(args.percentile_q)
    ...
    for img_path in images:
        ...
        process_image(
            ...
            percentile_q_x=args.percentile_q_x,
            percentile_q_y=args.percentile_q_y,
