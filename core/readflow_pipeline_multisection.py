...
def save_paragraph_artifacts(
    ...
    for p in paragraphs:
        ...
        box_w = max(1, x2 - x1 + 1)
        box_h = max(1, y2 - y1 + 1)
        pad_x = max(1, int(round(0.10 * box_w)))
        pad_y = max(1, int(round(0.10 * box_h)))
        xa = max(0, x1 - pad_x)
        ya = max(0, y1 - pad_y)
        xb = min(w - 1, x2 + pad_x)
        yb = min(h - 1, y2 + pad_y)
...
def ocr_paragraph_blocks(
    ...
    for p in paragraphs:
        row = dict(p)
        ex1, ey1, ex2, ey2 = [int(v) for v in row.get("expanded_crop_box_xyxy", row["paragraph_box_xyxy"])]
        ...
        roi = gray[ey1 : ey2 + 1, ex1 : ex2 + 1]
        roi_img = Image.fromarray(roi).convert("RGB")
        width_px = max(1, ex2 - ex1 + 1)
        est_chars = max(24, int(round(width_px / 5.5)))
        max_toks_para = max(80, min(max_tokens, int(round(est_chars * 10.0))))
        ...
        clean_1 = sanitize_paragraph_ocr_text(raw_1)
        if clean_1:
            row["ocr_text"] = clean_1
            out_rows.append(row)
            paragraph_texts.append(clean_1)
            continue
        ...
        clean_2 = sanitize_paragraph_ocr_text(raw_2)
        row["ocr_text"] = clean_2
        out_rows.append(row)
        paragraph_texts.append(clean_2)
...
def save_debug_line_model(
    ...
    for idx in range(total):
        ...
        if kind == "paragraphblank":
            ...
            color = BLANKLINE_COLOR
            label = f"BL{idx + 1}"
        else:
            color = (255, 255, 255)
            label = f"L{idx + 1}"
        cv2.rectangle(serialized_vis, (bx1, by1), (bx2, by2), color, 1)
        cv2.putText(
            serialized_vis,
            label,
...
def process_image(
    ...
    full_ocr_text = ""
    raw_ocr_source = "paragraph_blocks"
    line_indent_flags: list[bool] = []
    ...
    if paragraph_joined_ocr.strip():
        raw_ocr_source = "paragraph_blocks"
        ocr_text = paragraph_joined_ocr.rstrip()
    else:
